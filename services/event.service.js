...
async function update(req, result) {
  sql.query(
    "update events set Name=?,Description=?,Long_Description=?,File=? where Id = ? ",
    [
      req.body.Name,
...
async function getList(req, result) {
  ...
  sql.query(
    "select SQL_CALC_FOUND_ROWS Id,Name,Description,Long_Description,File from events  LIMIT ?, ?;SELECT FOUND_ROWS() as total;",
    [skip, pagesize],

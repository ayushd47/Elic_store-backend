...
async function getById(id,result) {
    sql.query("select Id,Name,Image from gallery where id = ? ", id, function (err, data) {             
        if(err) {
            result(err, null);
        }
        else{
...
async function update(req,result) {
    sql.query("update gallery set Name=?,Image=? where Id = ? ", [req.body.Name,req.body.Image,req.body.Id], function (err, data) {             
        if(err) {
            result(err, null);
        }
        else{
...
async function getList(req,result) {
    ...
    sql.query("select SQL_CALC_FOUND_ROWS Id, Name,Image from gallery LIMIT ?, ?;SELECT FOUND_ROWS() as total;",[skip,pagesize], function (err, response) {             

...
async function getById(id, result) {
  sql.query(
    "select Id,Name,Duration,File from course where id = ? ",
    id,
    function(err, data) {
      if (err) {
        result(err, null);
...
async function update(req, result) {
  sql.query(
    "update course set Name=?,Duration=?,File=? where Id = ? ",
    [req.body.Name, req.body.Duration, req.body.File, req.body.Id],
    function(err, data) {
      if (err) {
        result(err, null);
      } else {
...
async function getList(req, result) {
  ...
  sql.query(
    "select SQL_CALC_FOUND_ROWS Id, Name,Duration,File from course LIMIT ?, ?;SELECT FOUND_ROWS() as total;",
    [skip, pagesize],

...
async function create(req,result){
    let notices = {Id: uuidv1(),Name : req.body.Name,Description:req.body.Description,Long_Description:req.body.Long_Description,IsActive:1,File:req.body.File}
...
async function update(req,result) {
    ...
    sql.query("update notices set Name=?,Description=?,Long_Description=?,File=? where Id = ? ", [req.body.Name,req.body.Description,req.body.Long_Description,req.body.File,req.body.Id], function (err, data) {             
        if(err) {
            result(err, null);
        }
        else{
...
async function getList(req,result) {
    ...
    sql.query("select SQL_CALC_FOUND_ROWS Id,Name,Description,Long_Description,File from notices  LIMIT ?, ?;SELECT FOUND_ROWS() as total;",[skip,pagesize], function (err, response) {             
